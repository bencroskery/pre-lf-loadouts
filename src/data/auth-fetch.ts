...
export async function fetchWithBungieOAuth(
  ...
  if (await responseIndicatesBadToken(response)) {
    ...
    console.log('bungie auth', 'Access token expired, removing access token and trying again');
    removeAccessToken();
...
export async function getActiveToken(): Promise<Tokens> {
  ...
  try {
    ...
    return await handleRefreshTokenError(e as Error);
  }
}
  ...
  if (data) {
    if (data.error === 'server_error') {
      if (data.error_description === 'SystemDisabled') {
        throw new Error('Maintenance in progress, please try again later');
