import { dedupePromise } from '~/utils/util';
import { BUNGIE_API_KEY, oauthClientId, oauthClientSecret } from './bungie-api-utils';
  ...
  (refreshToken: Token): Promise<Tokens> => {
    ...
    return Promise.resolve(
      fetch(TOKEN_URL, {
        ...
        .then((token) => {
          setToken(token);
          console.log('bungie auth', 'Successfully updated auth token from refresh token.');
          return token;
        })
    );
  }
